...
public class HChannelImpl implements HChannel {
	 ...
	 *            Channel manager manages the channels.
	 */
	public HChannelImpl(InetSocketAddress host, Channel channel, AbstractHChannelManager channelManager,
			ClientChannelPipelineFactory pipelineFactory) {
		this.host = host;
	...
	private void executeOpAfterConnected(PubSubData pubSubData) {
		PubSubRequest.Builder reqBuilder = NetUtils.buildPubSubRequest(channelManager.nextTxnId(), pubSubData);
	...
	private void writePubSubRequest(PubSubData pubSubData, PubSubRequest pubSubRequest) {
		...
		try {
			getHChannelHandlerFromChannel(channel).addTxn(pubSubData.txnId, pubSubData);
	...
	protected void retryOrFailOp(PubSubData pubSubData) {
		...
		if (pubSubData.connectFailedServers != null && pubSubData.connectFailedServers.contains(hostString)) {
			...
			logger.error("Error connecting to host {} more than once so fail the request: {}", va(host, pubSubData));
			...
			System.out.println("Dns cache is disabled.");
	...
	/**
	 * This is a helper method to do the connect attempt to the server given the inputted host/port. This can be used to
	 * connect to the default server host/port which is the VIP. That will pick a server in the cluster at random to
	 * connect to for the initial PubSub attempt (with redirect logic being done at the server side). Additionally, this
	 * could be called after the client makes an initial PubSub attempt at a server, and is redirected to the one that
	 * is responsible for the topic. Once the connect to the server is done, we will perform the corresponding PubSub
	 * write on that channel.
	 * 
	 * @param serverHost
	 *            Input server host to connect to of type InetSocketAddress
	 * @param pipelineFactory
	 *            PipelineFactory to create response handler to handle responses from underlying channel.
	 */
	...
	public void close(boolean wait) {
		...
		try {
			...
			logger.warn("No channel handler found for channel {} when closing it.", channel);
		}
		if (wait) {
	...
	public String toString() {
		...
		sb.append("[HChannel: host - ").append(host).append(", channel - ").append(channel).append(", pending reqs - ")
				.append(pendingOps.size()).append(", closed - ").append(closed).append("]");
		return sb.toString();
	}
	...
	/**
	 * Helper static method to get the ResponseHandler instance from a Channel via the ChannelPipeline it is associated
	 * with. The assumption is that the last ChannelHandler tied to the ChannelPipeline is the ResponseHandler.
	 * 
	 * @param channel
	...
	public static HChannelHandler getHChannelHandlerFromChannel(Channel channel) throws NoResponseHandlerException {
		if (null == channel) {
			throw new NoResponseHandlerException(
					"Received a null value for the channel. Cannot retrieve the response handler");
		}
