...
public abstract class AbstractHChannelManager implements HChannelManager {
	...
	// Empty Topic List
	private final static Set<ByteString> EMPTY_TOPIC_SET = new HashSet<ByteString>();
	...
	protected final CleanupChannelMap<InetSocketAddress> host2NonSubscriptionChannels = new CleanupChannelMap<InetSocketAddress>();
	...
	protected AbstractHChannelManager(ClientConfiguration cfg, ChannelFactory socketFactory) {
		...
		defaultServerChannel = new DefaultServerChannel(cfg.getDefaultServerHost(), this);
	...
	public void schedule(final TimerTask task, final long delay) {
		...
		try {
			if (closed) {
				logger.warn("Task {} is not scheduled due to the channel manager is closed.", task);
				return;
			}
	...
	public void submitOpAfterDelay(final PubSubData pubSubData, final long delay) {
		...
		try {
			...
			clientTimer.schedule(new TimerTask() {
				@Override
				public void run() {
					logger.debug("Submit request {} in {} ms later.", va(pubSubData, delay));
					submitOp(pubSubData);
				}
			}, delay);
		} finally {
	...
	public void submitOp(PubSubData pubSubData) {
		HChannel hChannel;
		// lizhhb for test, add "OperationType.QUEUE_TOPIC_OP"
	...
	public void redirectToHost(PubSubData pubSubData, InetSocketAddress host) {
		logger.debug("Submit operation {} to host {}.", va(pubSubData, host));
		HChannel hChannel;
		if (OperationType.PUBLISH.equals(pubSubData.operationType)
				|| OperationType.UNSUBSCRIBE.equals(pubSubData.operationType)) {
	...
	void submitOpThruChannel(PubSubData pubSubData, Channel channel) {
		logger.debug("Submit operation {} to thru channel {}.", va(pubSubData, channel));
		HChannel hChannel;
		/* msgbus team: add a type QUEUE_TOPIC_OP */
		if (OperationType.PUBLISH.equals(pubSubData.operationType)
				|| OperationType.UNSUBSCRIBE.equals(pubSubData.operationType)
				|| OperationType.QUEUE_TOPIC_OP.equals(pubSubData.operationType)) {
	...
	public void submitOpToDefaultServer(PubSubData pubSubData) {
		logger.debug("Submit operation {} to default server {}.", va(pubSubData, defaultServerChannel));
	...
	private HChannel createAndStoreNonSubscriptionChannel(Channel channel) {
		...
		HChannel newHChannel = new HChannelImpl(host, channel, this, getNonSubscriptionChannelPipelineFactory());
	...
	private HChannel createAndStoreNonSubscriptionChannel(InetSocketAddress host) {
		HChannel newHChannel = new HChannelImpl(host, this, getNonSubscriptionChannelPipelineFactory());
		return storeNonSubscriptionChannel(host, newHChannel);
	}
	 ...
	 * @param topic
	 *            Topic Name
	 * @return if <code>topic</code>'s owner is unknown, return null. if <code>topic</code>'s owner is know and there is
	 *         a channel existed before, return the existed channel, otherwise created a new one.
	 */
	...
	protected void onNonSubscriptionChannelDisconnected(InetSocketAddress host, Channel channel) {
		...
		logger.info("NonSubscription Channel {} to {} disconnected.", va(channel, host));
	...
	/**
	 * Create and store a subscription {@link HChannel} thru the underlying established <code>channel</code>
	 * 
	 * @param channel
	 ...
	 *            Topic Subscriber
	 * @return if <code>topic</code>'s owner is unknown, return null. if <code>topic</code>'s owner is know and there is
	 *         a channel existed before, return the existed channel, otherwise created a new one for the
	 ...
	 *            The underlying established channel.
	 */
	protected abstract void onSubscriptionChannelDisconnected(InetSocketAddress host, Channel channel);

	private void sendConsumeRequest(final TopicSubscriber topicSubscriber, final MessageSeqId messageSeqId,
			final Channel channel) {
		PubSubRequest.Builder pubsubRequestBuilder = NetUtils.buildConsumeRequest(nextTxnId(), topicSubscriber,
				messageSeqId);
	...
	/**
	 * Helper method to store the topic2Host mapping in the channel manager cache map. This method is assumed to be
	 * called when we've done a successful connection to the correct server topic master.
	 * 
	 * @param topic
	 *            Topic Name
	 * @param host
	...
	protected void storeTopic2HostMapping(ByteString topic, InetSocketAddress host) {
		...
		if (null != oldHost) {
			...
			logger.debug("Storing info for topic: {}, host: {}.", va(topic.toStringUtf8(), host));
		}
		...
		if (null != topicsForHost) {
			synchronized (topicsForHost) {
				for (ByteString topic : topicsForHost) {
					logger.debug("Removing mapping for topic: {} from host: {}.", va(topic.toStringUtf8(), host));
	...
	public void clearHostForTopic(ByteString topic, InetSocketAddress host) {
		logger.debug("Clearing topic: {} from host: {}.", va(topic.toStringUtf8(), host));
		if (topic2Host.remove(topic, host)) {
			logger.debug("Removed topic to host mapping for topic: {} and host: {}.", va(topic.toStringUtf8(), host));
		}
		Set<ByteString> topicsForHost = host2Topics.get(host);
		if (null != topicsForHost) {
			boolean removed;
			...
			if (removed) {
				logger.debug("Removed topic: {} from host: {}.", topic.toStringUtf8(), host);
	...
	/* msgbus team */
	protected void updateHosts(String hostString) {
		// this.hosts=hosts;
		hosts.clear();
		String[] elements = hostString.split(",");
		for (int i = 0; i < elements.length; i++) {
			hosts.add(elements[i]);
		}
