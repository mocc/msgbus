...
public class SubscriptionChannelManager implements ChannelDisconnectListener {
	...
	/* msgbus  team */
	DeliveryManager dm;
	public SubscriptionChannelManager(DeliveryManager dm) {
		sub2Channel = new ConcurrentHashMap<TopicSubscriber, Channel>();
		channel2sub = new ConcurrentHashMap<Channel, Set<TopicSubscriber>>();
		this.dm=dm;
	}
	...
	public void channelDisconnected(Channel channel) {
		...
		if (topicSubs != null) {
			for (TopicSubscriber topicSub : topicSubs) {
				...
				/* msgbus  team */				
				dm.channelDisconnected(topicSub.getTopic(), topicSub.getSubscriberId(),channel);						        
		        /* msgbus  team */
	...
	public Channel put(TopicSubscriber topicSub, Channel channel, boolean forceAttach) {
		...
		synchronized (channel) {
			...
			if (null != oldChannel && !oldChannel.equals(channel)) {
				...
				if (forceAttach) {
					// it is safe to close old subscription here since the new
					// subscription
					...
					synchronized (oldChannel) {
						...
						if (null != oldTopicSubs) {
							if (!oldTopicSubs.remove(topicSub)) {
								logger.warn(
										"Failed to remove old subscription ({}) due to it isn't on channel ({}).",
					...
					logger.info(
							"Subscribe request for ({}) from channel ({}) closes old subscripiton on channel ({}).",
							va(topicSub, channel, oldChannel));
					// try replace the oldChannel
					// if replace failure, it migth caused because
					// channelDisconnect callback
					// has removed the old channel.
					if (!sub2Channel.replace(topicSub, oldChannel, channel)) {
						// try to add it now.
						// if add failure, it means other one has obtained the
						// channel
				...
				if (!subSuccess) {
					logger.error(
							"Error serving subscribe request for ({}) from ({}) since it already served on ({}).",
							va(topicSub, channel, oldChannel));
					return oldChannel;
				}
			}
