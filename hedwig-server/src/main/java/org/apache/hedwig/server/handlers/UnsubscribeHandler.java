...
public class UnsubscribeHandler extends BaseHandler {	
	...
	public UnsubscribeHandler(ServerConfiguration cfg, TopicManager tm,
			SubscriptionManager subMgr, DeliveryManager deliveryMgr,
			SubscriptionChannelManager subChannelMgr) {
		super(tm, cfg);
		this.subMgr = subMgr;
		...
		subMgr.unsubscribe(topic, subscriberId, new Callback<Void>() {
			...
			public void operationFailed(Object ctx, PubSubException exception) {
				...
				if(topic.toStringUtf8().startsWith(SubscribeHandler.QUEUE_PRIFIX)
						&& (StatusCode.CLIENT_NOT_SUBSCRIBED == exception.getCode())) {
					...
					if(true == deliveryMgr.deleteTopicPersistenceInfoRecursive(topic)) {
						channel.write(PubSubResponseUtils.getSuccessResponse(request
								.getTxnId()));						
					}
					else {
				...
				/* msgbus  team */
				channel.write(PubSubResponseUtils.getResponseForException(exception,
						request.getTxnId()));
				unsubStats.incrementFailedOps();
			}
						...
						new Callback<Void>() {
							...
							public void operationFinished(Object ctx, Void resultOfOperation) {
								...
								subChannelMgr.remove(new TopicSubscriber(topic, subscriberId),
										channel);								
								//Send respnse only once 
								/*channel.write(PubSubResponseUtils.getSuccessResponse(request
										.getTxnId()));*/
								unsubStats.updateLatency(System.currentTimeMillis()
										- requestTime);	
