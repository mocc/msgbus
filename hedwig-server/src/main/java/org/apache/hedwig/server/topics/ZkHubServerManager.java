...
class ZkHubServerManager implements HubServerManager {
	...
	public ZkHubServerManager(ServerConfiguration conf, ZooKeeper zk, HedwigSocketAddress addr) {
		this.conf = conf;
		this.zk = zk;
		this.addr = addr;
	...
	private String getHubZkNodePath(HedwigSocketAddress node) {
		String nodePath = this.conf.getZkHostsPrefix(new StringBuilder()).append("/").append(node).toString();
		return nodePath;
	}
								...
								public void safeProcessResult(int rc, String path, Object ctx, Stat stat) {
									if (rc == Code.OK.intValue()) {
										...
										callback.operationFailed(ctx, new PubSubException.ServiceDownException(
												"I can't state my hub node after I created it : " + ephemeralNodePath));
										return;
									}
								}
							}, ctx);
	...
	public void uploadSelfLoadData(HubLoad selfLoad) {
		...
		zk.setData(ephemeralNodePath, loadDataBytes, -1, loadReportingStatCallback, null);
	}
	...
	public void chooseLeastLoadedHub(final Callback<HubInfo> callback, Object ctx) {
		...
		zk.getChildren(hubNodesPath, false, new SafeAsyncZKCallback.ChildrenCallback() {
			...
			public void safeProcessResult(int rc, String path, Object ctx, List<String> children) {
				if (rc != Code.OK.intValue()) {
					KeeperException e = ZkUtils.logErrorAndCreateZKException("Could not get list of available hubs",
							path, rc);
	...
	private void chooseLeastLoadedNode(final List<String> children, final Callback<HubInfo> callback, Object ctx) {
			...
			public void safeProcessResult(int rc, String path, Object ctx, byte[] data, Stat stat) {
				synchronized (this) {
					...
					if (numResponses == children.size()) {
						if (leastLoaded == null) {
							callback.operationFailed(ctx, new PubSubException.ServiceDownException("No hub available"));
							return;
						}
						try {
							...
							callback.operationFailed(ctx, new PubSubException.ServiceDownException(
									"Least loaded hub server " + leastLoaded + " is invalid."));
						}
					}
				}
			}
