...
public class FIFODeliveryManager implements Runnable, DeliveryManager, SubChannelDisconnectedListener {
    ...
    public class ActiveSubscriberState
        ...
        public void messageScanned(Object ctx, Message message) {
            ...
            // test
            /*if (ctx == null) {
                logger.info("Message to send: " + message.getMsgId().getLocalComponent());
            } else {
    ...
    public class QueueConsumer implements DeliveryCallback {
        ...
        public void permanentErrorOnSend(Object ctx) {
            ...
            enqueueWithoutFailure(new RemoveChannelForQueueRequest(new TopicSubscriber(ass.topic, ass.subscriberId), endPoint.channel));
        }
    ...
    protected class ConsumerCluster {
        ...
        public void addConsumeSeqIdToCluster(ByteString topic, ByteString subscriberId, MessageSeqId consumeSeqId,
            ...
            if (qc == null) {
                // disconnected, the message will be resent, but it's second consume will be omitted
                return;
            }
            ...
            /*
             * if(subscriberId.equals(SubscriptionStateUtils.QUEUE_SUBID_BS)) { ((ReadAheadCache)
             * persistenceMgr).removeMessageFromCache(topic, longConsumeSeqId); }
             */
