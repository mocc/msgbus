...
public class FIFODeliveryManager implements Runnable, DeliveryManager,
	...
	protected static final Logger logger = LoggerFactory
			.getLogger(FIFODeliveryManager.class);
	 ...
	 */
	Queue<ActiveSubscriberState> retryQueue = new PriorityBlockingQueue<ActiveSubscriberState>(
			32, new Comparator<ActiveSubscriberState>() {
				@Override
				public int compare(ActiveSubscriberState as1,
						ActiveSubscriberState as2) {
	...
	public FIFODeliveryManager(PersistenceManager persistenceMgr,
	...
	/* msgbus add --> */
	public FIFODeliveryManager(PersistenceManager persistenceMgr, ZooKeeper zk,
			ServerConfiguration cfg) {
		this.persistenceMgr = persistenceMgr;
		if (persistenceMgr instanceof ReadAheadCache) {
			this.cache = (ReadAheadCache) persistenceMgr;
	...
	protected void enqueueWithoutFailure(DeliveryManagerRequest request) {
		if (!requestQueue.offer(request)) {
			throw new UnexpectedError("Could not enqueue object: " + request
					+ " to delivery manager request queue.");
		}
	}
	 ...
	 */
	@Override
	public void startServingSubscription(ByteString topic,
			ByteString subscriberId, SubscriptionPreferences preferences,
			MessageSeqId seqIdToStartFrom, DeliveryEndPoint endPoint,
			ServerMessageFilter filter, Callback<Void> callback, Object ctx) {
		ActiveSubscriberState subscriber = new ActiveSubscriberState(topic,
	...
	public void stopServingSubscriber(ByteString topic,
		...
		enqueueWithoutFailure(new StopServingSubscriber(topic, subscriberId,
				event, cb, ctx));
	}
	 ...
	 * @param subscriber
	 */
	public void retryErroredSubscriberAfterDelay(
		...
		if (!retryQueue.offer(subscriber)) {
			throw new UnexpectedError(
					"Could not enqueue to delivery manager retry queue");
		}
	}
	...
	public void messageConsumed(ByteString topic, ByteString subscriberId,
		...
		ActiveSubscriberState subState = subscriberStates
				.get(new TopicSubscriber(topic, subscriberId));
		if (null == subState) {
			return;
		}
	...
	public void moveDeliveryPtrForward(ActiveSubscriberState subscriber,
			long prevSeqId, long newSeqId) {
		enqueueWithoutFailure(new DeliveryPtrMove(subscriber, prevSeqId,
				newSeqId));
	}
	...
	protected void retryErroredSubscribers() {
		long lastInterestingFailureTime = MathUtils.now()
				- cfg.getScanBackoffPeriodMs();
		ActiveSubscriberState subscriber;
	...
	protected void removeDeliveryPtr(ActiveSubscriberState subscriber,
			Long seqId, boolean isAbsenceOk, boolean pruneTopic) {

		assert seqId != null;

		// remove this subscriber from the delivery pointers data structure
		ByteString topic = subscriber.getTopic();
		SortedMap<Long, Set<ActiveSubscriberState>> deliveryPtrs = perTopicDeliveryPtrs
				.get(topic);

		if (deliveryPtrs == null && !isAbsenceOk) {
			throw new UnexpectedError(
					"No delivery pointers found while disconnecting "
		...
		if (!MapMethods.removeFromMultiMap(deliveryPtrs, seqId, subscriber)
				&& !isAbsenceOk) {

			throw new UnexpectedError("Could not find subscriber:" + subscriber
					+ " at the expected delivery pointer");
		}
	...
	protected void addDeliveryPtr(ActiveSubscriberState subscriber, Long seqId) {
		...
		SortedMap<Long, Set<ActiveSubscriberState>> deliveryPtrs = MapMethods
				.getAfterInsertingIfAbsent(perTopicDeliveryPtrs,
						subscriber.getTopic(),
						TreeMapLongToSetSubscriberFactory.instance);

		MapMethods.addToMultiMap(deliveryPtrs, seqId, subscriber,
				HashMapSubscriberFactory.instance);
	}

	public class ActiveSubscriberState implements ScanCallback,
			DeliveryCallback, DeliveryManagerRequest, CancelScanRequest {
		...
		public ActiveSubscriberState(ByteString topic, ByteString subscriberId,
			...
			if (preferences.hasMessageWindowSize()) {
				...
				if (FIFODeliveryManager.this.cfg.getDefaultMessageWindowSize() > 0) {
					messageWindowSize = FIFODeliveryManager.this.cfg
							.getDefaultMessageWindowSize();
				} else {
		...
		public void setNotConnected(SubscriptionEvent event) {
			...
			if (null != event
				...
				// multiplexing
				PubSubResponse response = PubSubResponseUtils
						.getResponseForSubscriptionEvent(topic, subscriberId,
								event);
		...
		protected synchronized void messageConsumed(long newSeqIdConsumed) {
			...
			if (logger.isDebugEnabled()) {
				logger.debug(
						"Subscriber ({}) moved consumed ptr from {} to {}.",
		...
		public void deliverNextMessage() {
			...
			if (!delivering.get()) {
				logger.info("Stop delivering next message. count= "
						+ (++consumerCluster.stopCount));
				return;
			}
			/* <--msgbus added add isThrottled will be ineffective--> */
		...
		private void doDeliverNextMessage() {
			...
			synchronized (this) {
				...
				localSeqIdDeliveringNow = persistenceMgr.getSeqIdAfterSkipping(
						topic, lastLocalSeqIdDelivered, 1);

				outstandingScanRequest = new ScanRequest(topic,
		...
		public void messageScanned(Object ctx, Message message) {
			...
			DeliveryEndPoint queueEndPoint = ((ctx == null) ? qc.endPoint
					: qc.endPoint2);
			// test
			/*
			...
			// between them
			qc.unConsumedMsgSeqs.add(new UnConsumedSeq(message.getMsgId()
					.getLocalComponent(), System.currentTimeMillis()));
		...
		public void sendingFinished() {
			...
			synchronized (this) {
				...
				if (lastLocalSeqIdDelivered > lastSeqIdCommunicatedExternally
						+ SEQ_ID_SLACK) {
					...
					moveDeliveryPtrForward(this, prevId,
		...
		public void performRequest() {
			...
			if ((ass = subscriberStates.get(ts)) == null
					|| ass.connected == false) {
				consumerCluster = new ConsumerCluster(this,
						lastLocalSeqIdDelivered);
				QueueConsumer qc = new QueueConsumer(
						(ChannelEndPoint) this.deliveryEndPoint,
						consumerCluster, this, this.messageWindowSize);
				...
				QueueConsumer qc = new QueueConsumer(
						(ChannelEndPoint) this.deliveryEndPoint,
						consumerCluster, ass, this.messageWindowSize);
				consumerCluster.addNewConsumer(qc);
	...
	protected class StopServingSubscriber implements DeliveryManagerRequest {
		...
		public StopServingSubscriber(ByteString topic, ByteString subscriberId,
				SubscriptionEvent event, Callback<Void> callback, Object ctx) {
	 ...
	 */
	private void doStopServingSubscriber(ActiveSubscriberState subscriber,
			SubscriptionEvent event) {
		...
		removeDeliveryPtr(subscriber,
				subscriber.getLastSeqIdCommunicatedExternally(), //
				// isAbsenceOk=
				true,
				// pruneTopic=
				true);
	...
	protected class DeliveryPtrMove implements DeliveryManagerRequest {
		...
		public DeliveryPtrMove(ActiveSubscriberState subscriber, Long oldSeqId,
				Long newSeqId) {
	...
	protected class ShutdownDeliveryManagerRequest implements
			DeliveryManagerRequest {
	...
	protected static class HashMapSubscriberFactory implements
			Factory<Set<ActiveSubscriberState>> {
	...
	public void onSubChannelDisconnected(TopicSubscriber topicSubscriber,
			Channel channel) {
		/* msgbus modified */
		enqueueWithoutFailure(new RemoveChannelForQueueRequest(topicSubscriber,
				channel));
		/*
	...
	public class QueueConsumer implements DeliveryCallback {
		...
		public QueueConsumer(ChannelEndPoint deliveryEndPoint,
				ConsumerCluster cc, ActiveSubscriberState ass,
				int messageWindowSize) {
		...
		public void sendingFinished() {
			...
			synchronized (ass) {
				...
				if (ass.lastLocalSeqIdDelivered > ass.lastSeqIdCommunicatedExternally
					...
					moveDeliveryPtrForward(ass, prevId,
							ass.lastLocalSeqIdDelivered);
				}
				// increment deliveried message
				ServerStats.getInstance().incrementMessagesDelivered();
		...
		@Override
		public void permanentErrorOnSend(Object ctx) {
			// The unConsumed messages will be moved to retryMessageQueue when
			// the request is processed, and
			// deliverNextMessage should be called after that moment
			enqueueWithoutFailure(new RemoveChannelForQueueRequest(
					new TopicSubscriber(ass.topic, ass.subscriberId),
					endPoint.channel));
		}
	...
	protected class ConsumerCluster {
		 ...
		 * 
		 */
		TreeSet<SeqBlock> consumedSeqs = new TreeSet<SeqBlock>(
				new Comparator<SeqBlock>() {
		...
		public void addConsumeSeqIdToCluster(ByteString topic,
				ByteString subscriberId, MessageSeqId consumeSeqId,
				AbstractSubscriptionManager sm, Callback<Void> callback,
				Object ctx) {
			// test
			// logger.info("Consumed seq: " + consumeSeqId.getLocalComponent() +
			// ". lastConsumedSeq: " +
			// lastConsumedSeq);
			...
			if (addToListAndMaybeConsume(longConsumeSeqId)) {
				sm.setConsumeSeqIdForSubscriber(topic, subscriberId,
						MessageSeqId.newBuilder(consumeSeqId)
								.setLocalComponent(lastConsumedSeq).build(),
						callback, null);
			}
	...
	protected class AddConsumeSeqIdRequest implements DeliveryManagerRequest {
		...
		public AddConsumeSeqIdRequest(ByteString topic,
				ByteString subscriberId, MessageSeqId consumeSeqId,
				AbstractSubscriptionManager sm, Callback<Void> callback,
				Object ctx) {
			this.topic = topic;
		...
		@Override
		public void performRequest() {
			ConsumerCluster cc = subscriberStates.get(new TopicSubscriber(
					topic, subscriberId)).consumerCluster;
			cc.addConsumeSeqIdToCluster(topic, subscriberId, consumeSeqId, sm,
					callback, ctx);
		}
	}
	...
	public void addConsumeSeqForQueue(ByteString topic,
		...
		enqueueWithoutFailure(new AddConsumeSeqIdRequest(topic, subscriberId,
				consumeSeqId, sm, callback, ctx));
	}

	protected class RemoveChannelForQueueRequest implements
			DeliveryManagerRequest {
		TopicSubscriber topicSubscriber;
		Channel channel;

		public RemoveChannelForQueueRequest(TopicSubscriber topicSubscriber,
				Channel channel) {
		...
		public void performRequest() {
			...
			if (cc.allConsumers.isEmpty()) {
				logger.info("Subscriber {} will be removed.",
						va(topicSubscriber));
				// test
				...
				// it,s ok?
				stopServingSubscriber(topicSubscriber.getTopic(),
						topicSubscriber.getSubscriberId(), null, NOP_CALLBACK,
						null);
			}
		}
	}
				...
				for (Channel channel : cc.allConsumers.keySet()) {
					...
					while (!qc.unConsumedMsgSeqs.isEmpty()) {
						...
						if (unConsumedSeq.timeStamp + cc.timeOut > currentTime)
							break;
						logger.info("Move expired message to retryMessageQueue: "
								+ unConsumedSeq.seq);
	...
	public boolean deleteTopicPersistenceInfoRecursive(ByteString topic) {
		long currentSeqId = 0;
		try {
			currentSeqId = persistenceMgr.getCurrentSeqIdForTopic(topic)
					.getLocalComponent();
