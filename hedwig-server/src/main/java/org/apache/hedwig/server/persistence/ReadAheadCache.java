...
public class ReadAheadCache implements PersistenceManager, Runnable, HedwigJMXService {
	...
	/**
	 * The underlying persistence manager that will be used for persistence and scanning below the cache
	 */
	...
	/**
	 * To simplify synchronization, the cache will be maintained by a single cache maintainer thread. This is the queue
	 * that will hold requests that need to be served by this thread
	 */
	protected BlockingQueue<CacheRequest> requestQueue = new LinkedBlockingQueue<CacheRequest>();

	/**
	 * We want to keep track of when entries were added in the cache, so that we can remove them in a FIFO fashion
	 */
	protected SortedMap<Long, Set<CacheKey>> timeIndexOfAddition = new TreeMap<Long, Set<CacheKey>>();

	/**
	 * We also want to track the entries in seq-id order so that we can clean up entries after the last subscriber
	 */
	protected Map<ByteString, SortedSet<Long>> orderedIndexOnSeqId = new HashMap<ByteString, SortedSet<Long>>();

	/**
	 * We maintain an estimate of the current size of the cache, so that we know when to evict entries.
	 */
	protected AtomicLong presentCacheSize = new AtomicLong(0);

	/**
	 * One instance of a callback that we will pass to the underlying persistence manager when asking it to persist
	 * messages
	 */
	...
	/**
	 * ======================================================================== Methods of {@link PersistenceManager}
	 * that we will pass straight down to the real persistence manager.
	 */
	...
	/**
	 * ======================================================================== Other methods of
	 * {@link PersistenceManager} that the cache needs to take some action on.
	 * 
	 * 1. Persist: We pass it through to the real persistence manager but insert our callback on the return path
	 * 
	 */
	...
	/**
	 * The callback that we insert on the persist request return path. The callback simply forms a
	 * {@link PersistResponse} object and inserts it in the request queue to be handled serially by the cache maintainer
	 * thread.
	 * 
	 */
	public class PersistCallback implements Callback<PubSubProtocol.MessageSeqId> {
		...
		/**
		 * In case there is a failure in persisting, just pass it to the original callback
		 */
		...
		/**
		 * When the persist finishes, we first notify the original callback of success, and then opportunistically treat
		 * the message as if it just came in through a scan
		 */
	...
	/**
	 * Too complicated to deal with enqueue failures from the context of our callbacks. Its just simpler to quit and
	 * restart afresh. Moreover, this should not happen as the request queue for the cache maintainer is unbounded.
	 * 
	 * @param obj
	 */
	 ...
	 * 
	 * 2. Scan - Since the scan needs to touch the cache, we will just enqueue the scan request and let the cache
	 * maintainer thread handle it.
	 */
	 ...
	 * Another method from {@link PersistenceManager}.
	 * 
	 * Since this is a cache layer on top of an underlying persistence manager, we can just call the consumedUntil
	 * method there. The messages older than the latest one passed here won't be accessed anymore so they should just
	 * get aged out of the cache eventually. For now, there is no need to proactively remove those entries from the
	 * cache.
	 */
	...
	/**
	 * ======================================================================== BEGINNING OF CODE FOR THE CACHE
	 * MAINTAINER THREAD
	 * 
	 * 1. The run method. It simply dequeues from the request queue, checks the type of object and acts accordingly
	 */
	public void run() {
		while (keepRunning) {
			CacheRequest obj;
	...
	/**
	 * The readahead policy is simple: We check if an entry already exists for the message being requested. If an entry
	 * exists, it means that either that message is already in the cache, or a read for that message is outstanding. In
	 * that case, we look a little ahead (by readAheadCount/2) and issue a range read of readAheadCount/2 messages. The
	 * idea is to ensure that the next readAheadCount messages are always available.
	 * 
	...
	/**
	 * This method just checks if the provided seq-id already exists in the cache. If not, a range read of the specified
	 * amount is issued.
	 * 
	 * @param topic
	 * @param seqId
	...
	/**
	 * For adding the message to the cache, we do some bookeeping such as the total size of cache, order in which
	 * entries were added etc. If the size of the cache has exceeded our budget, old entries are collected.
	 * 
	 * @param cacheKey
	 * @param message
	 */
	...
	protected void removeMessageFromCache(final CacheKey cacheKey, Exception exception,
		...
		logger.debug("remove msg {} from cache", cacheKey);
			...
			enqueueWithoutFailure(new CacheRequest() {
				...
				public void performRequest() {
					...
					if (maintainTimeIndex) {
						MapMethods.removeFromMultiMap(timeIndexOfAddition, additionTime, cacheKey);
					}
				}
			});
		}
	...
	/**
	 * Collection of old entries is simple. Just collect in insert-time order, oldest to newest.
	 */
	protected void collectOldCacheEntries() {
		while (presentCacheSize.get() > cfg.getMaximumCacheSize() && !timeIndexOfAddition.isEmpty()) {
	...
	protected class ExceptionOnCacheKey implements CacheRequest {
		...
		/**
		 * If for some reason, an outstanding read on a cache stub fails, exception for that key is enqueued by the
		 * {@link ReadAheadScanCallback}. To handle this, we simply send error on the callbacks registered for that
		 * stub, and delete the entry from the cache
		 */
		public void performRequest() {
	...
	protected class ScanRequestWrapper implements CacheRequest {
		...
		/**
		 * To handle a scan request, we first try to do readahead (which might cause a range read to be issued to the
		 * underlying persistence manager). The readahead will put a stub in the cache, if the message is not already
		 * present in the cache. The scan callback that is part of the scan request is added to this stub, and will be
		 * called later when the message arrives as a result of the range scan issued to the underlying persistence
		 * manager.
		 */
	...
	/**
	 * msgbus  team: a public method, in order to maintain the cache
	 */
	public void removeMessageFromCache(ByteString topic, long seqId) {
		removeMessageFromCache(new CacheKey(topic, seqId), readAheadExceptionInstance, //
				// maintainTimeIndex=
				true,
				// maintainSeqIdIndex=
				true);
		if (orderedIndexOnSeqId.isEmpty()) {
			orderedIndexOnSeqId.remove(topic);
		}
	}
}
